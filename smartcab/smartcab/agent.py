...
class LearningAgent(Agent):
	...
	if (testing == True): self.epsilon, self.alpha = 0.0, 0.0
	else: self.epsilon = math.exp(-self.alpha*self.trial)
	...
	if not state in self.Q: 
	    self.Q[state] = {}
	    for action in self.valid_actions: self.Q[state][action]=0.0
        return
    ...
    def choose_action(self, state):
	    ...
	    prob = random.random()
	    if (prob < self.epsilon): action = random.choice(self.valid_actions)
